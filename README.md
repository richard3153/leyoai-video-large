...
LeyoAI Video Model (Large) 是基于 Qwen2.5-7B-Instruct 微调的视频分析助手，提供最强的视频理解能力。
...
model = AutoModelForCausalLM.from_pretrained(
    "FFZwai/leyoai-video-large",
    device_map="auto"
)
tokenizer = AutoTokenizer.from_pretrained("FFZwai/leyoai-video-large")

# Inference
prompt = "进行深度视频内容分析，包括情感、场景、对象和事件检测"
...
LeyoAI Video Model (Large) is a video analysis assistant fine-tuned from Qwen2.5-7B-Instruct, providing the strongest video understanding capabilities.
...
# Inference
prompt = "Perform deep video content analysis including sentiment, scenes, objects, and event detection"
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
outputs = model.generate(**inputs, max_length=100)
response = tokenizer.decode(outputs[0], skip_special_tokens=True)
print(response)
